...
class Twitter:
    ...
    def authorize_twitter(apikey, apisec):
        # Authorize
        # access_token = ''
        # access_token_secret = ''
        ...
        auth_param_str = '&'.join(['{}={}'.format(urllib.parse.quote(str(key), ''), urllib.parse.quote(str(auth_param[key]), ''))
                                   for key in sorted(auth_param)])
        message = '{}&{}&{}'.format(
            'POST', urllib.parse.quote(req_url, ''), urllib.parse.quote(auth_param_str))
        key = '{}&{}'.format(apisec, '')
        signature = hmac.new(
        ...
        res = urllib.request.urlopen(
            req_url, data=urllib.parse.urlencode(auth_param).encode('ascii')).read()

        oauth_token = {key.split('=')[0]: key.split(
            '=')[1] for key in res.decode('ascii').split('&')}
        ...
        auth_param_str = '&'.join(['{}={}'.format(urllib.parse.quote(str(key), ''), urllib.parse.quote(str(auth_param[key]), ''))for key in sorted(auth_param)])
        message = '{}&{}&{}'.format(
            'POST', urllib.parse.quote(req_url, ''), urllib.parse.quote(auth_param_str))
        key = '{}&{}'.format(apisec, '')
        signature = hmac.new(
        ...
        digest_base64 = base64.encodestring(
            signature.digest()).decode('ascii').strip()
        auth_param['oauth_signature'] = digest_base64
        access_token_req = urllib.request.Request(oauth_token_url, data=urllib.parse.urlencode(
            auth_param).encode('utf-8'), headers={'Authorization': 'OAuth'})
        res = urllib.request.urlopen(access_token_req).read()
        oauth_token = {key.split('=')[0]: key.split(
            '=')[1] for key in res.decode('ascii').split('&')}
        return oauth_token

    def request(apikey, apisec, oauth_token, oauth_token_secret, url, content, method):
        param = {
            'oauth_consumer_key': apikey,
            'oauth_signature_method': 'HMAC-SHA1',
            'oauth_timestamp': int(time.time()),
        ...
        key = '{}&{}'.format(apisec, oauth_token_secret)
        signature = hmac.new(
            key.encode('utf-8'), message.encode('utf-8'), hashlib.sha1)
        digest_base64 = base64.encodestring(
            signature.digest()).decode('ascii').strip()
